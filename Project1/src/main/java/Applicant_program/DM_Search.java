...
public class DM_Search extends DM_Menu {
	
public void dmSearch() {
	
		do {
			System.out.println();
			System.out.println("------------------ 서류 합격 조회 ------------------");
			do {
			try {
				System.out.println("지원자 번호를 입력하십시오.");
		...
		String dm_sql2 = "select DB_number, DB_name from Applicant where DB_number = "+input_number;
		
		try {
			pstmt = conn.prepareStatement(dm_sql2);
	...
	try {
		...
		//3. sql 구문
		String dm_sql3 = "select * from Applicant where DB_number = ?";
		pstmt = conn.prepareStatement(dm_sql3);
		...
		while(rs.next()) {
			...
			int sum1 = (appli_career2 + appli_license2 + appli_assay2);
